...
const prompt = ai.definePrompt({
  ...
  2.  **Analyze Verification Result:**
      - If the tool returns 'false', it means the email is from a disposable service. In this case, you MUST stop processing. Your entire response should be a JSON object where the 'notes' field explains that disposable emails are not allowed and the other fields can be "N/A".
  ...
  4.  **Provide Estimate:** Based on these details, provide a single estimated "up to" cost. The cost should be reasonable and friendly for a small-to-medium sized agency in the Indian market. For example, "Up to ₹75,000".
  5.  **Create Breakdown:** Provide a convincing, detailed breakdown of the project modules and flow to instill confidence in the client. This should look like a mini-project-plan.
  6.  **Add Concluding Note:** Add a friendly note that this is a preliminary estimate and the price is negotiable after a more detailed discussion. Frame it as a starting point for a conversation.
